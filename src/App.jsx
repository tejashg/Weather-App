...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchapi = async () => {
      const url = `http://api.weatherapi.com/v1/current.json?key=275f76cece894961ab8180047223103&q=${city}`;
      try {
  ...
  return (
    <>
      <div className="container">
        <textarea placeholder="Search" onChange={(event) => setCity(event.target.value)} />
        ...
        <div className="credits">Made with <i className="fa-solid fa-heart"/> by Tejash Gupta</div>
      </div>
    </>
  );
};
